...
export default {
  user: {
    login: credentials => axios.post('/login', { credentials }).then(({ data }) => data.user),

    signup: user => axios.post('/signup', { user }).then(({ data }) => data.user),

    refresh: user => axios.post('/refresh', { user }).then(({ data }) => data.user),
  },
  ...
  contact: {
    ...
    save: async (contactInfo, aicode: string, orgIdEventId: any): Promise<any> =>
      axios({
        method: 'post',
  ...
  invoicing: {
    ...
    getSchoolList: ({ school, zipcode, city, state, ownAiCode }, source): Promise<any> =>
      axios
        .get(
          `${
            baseUrl.dev
